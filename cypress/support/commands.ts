...
Cypress.Commands.add('searchKlientWithinDistrict', (district, klientnavn) => {
    selectors.organisasjon().type(district);
    selectors.klientnavn().type(klientnavn);
    selectors.søk().click();
});

Cypress.Commands.add('searchKlient', (klientnavn) => {
    selectors.klientnavn().type(klientnavn);
    selectors.søk();
});

Cypress.Commands.add('registrerArbeidstasjon', (posisjon, klientnavn) => {
    selectors.registerArbeidstasjon().click();
    selectors.posisjon().clear().type(posisjon.toString());
    selectors.modalklientnavn().clear().type(klientnavn.toUpperCase());
    selectors.opprettArbeidstasjon().click();
});

Cypress.Commands.add('velgOrganisasjon', (district) => {
    selectors.organisasjon().select(district);
    //cy.wait(5000);
    //selectors.organisasjon().should('have.text', district);
});

Cypress.Commands.add('ÅpneRedigeringsVindu', (klientnavn) => {
    selectors.redigereArbeidstasjon(klientnavn.toUpperCase()).click();
});

Cypress.Commands.add('redigereArbeidstasjon', (posisjon, klientnavn) => {
    selectors.posisjon().clear().type(posisjon.toString());
    selectors.modalklientnavn().clear().type(klientnavn);
    selectors.lagreendringer().click();
    //Check the melding
});

Cypress.Commands.add('redigerePosisjonArbeidsstasjon', (posisjon) => {
    selectors.posisjon().clear().type(posisjon.toString());
    selectors.lagreendringer().click();
});

Cypress.Commands.add('redigereKlientArbeidsstasjon', (klientnavn) => {
    selectors.modalklientnavn().clear().type(klientnavn);
    selectors.lagreendringer().click();
});

Cypress.Commands.add('slettArbeidstasjon', (klientnavn) => {
    selectors.slettArbeidstasjon(klientnavn.toUpperCase()).click();
    selectors.bekreftSletting().click();
});
